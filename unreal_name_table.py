...
class UnrealNameTable:
	def __init__(self, file_handle, name_count, name_table_offset):
		super(UnrealNameTable, self).__init__()

		self.file_handle = file_handle
		self.count = name_count # int
		self.table_offset = name_table_offset # int

		self.file_handle.seek(self.table_offset)

		self.table = list()

		for i in range(0, self.count):
			self.temp_offset = self.file_handle.tell()
			self.temp_length = int.from_bytes(self.file_handle.read(1), ENDIANORDER)
			self.temp_name = self.file_handle.read(self.temp_length)
			self.temp_flags = self.file_handle.read(4)
